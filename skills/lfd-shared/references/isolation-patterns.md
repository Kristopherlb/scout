...
target-repo/                  agent's write surface, dev loop fully local
  .lfd/goal.md, .lfd/harness/score-dev.sh, .lfd/eval/dev/

eval-repo/                    private, zero agent access, single source of truth
  skills/lfd-design/, lfd-audit/, lfd-patch/ (hub-side only)
  targets/<name>/eval/holdout/, canary-list.json
  targets/<name>/harness/score-holdout.sh
...
2. Agent requests a holdout check by pushing an annotated tag
   (`<prefix>v1-<sha12>-<request-id>`) to target-repo. The tag message carries the full requested SHA and current
...
5. Pull the exact tagged SHA (no "latest HEAD" race — a tag is pinned by
   request), run `harness/score-holdout.sh` in the private hub, append one
